...
Committee.getCommittee <-
  function(committeeId) {
    ...
    Committee.getCommittee.basic <-
      function(.committeeId) {
        ...
        output <- lapply(output, FUN = function(i) {
          if (length(grep(pattern = "committeeId", x = names(i), value = TRUE)) > 0) {
            i
          }
          else {
            data.frame(i, "committeeId" = .committeeId, stringsAsFactors = FALSE)
          }
        })
    ...
    nodes <- lapply(output.list, FUN = function(x) {
      names(x)
    })
    nodes <- unique(unlist(nodes))

    output.list2 <- lapply(nodes, FUN = function(y) {
      lapply(output.list, FUN = function(x) {
        df. <- x[[y]]
    ...
    output.list3 <- lapply(output.list2, FUN = function(ol) {
      ...
      coln <- which.is.max(sapply(ol, function(x) {
        if (!is.null(x)) {
          ncol(x)
        } else {
          0
      ...
      max.cols <- max(sapply(ol, function(x) {
        if (!is.null(x)) {
          ncol(x)
        } else {
          0
    ...
    max.id <- which.is.max(sapply(output.list3, function(x) {
      dim(na.omit(x["committeeId"]))[1]
    }))
